...
# print(f"embedding length:{len(glove_model['frog'])}")
...
adv_train_data = []
train_n = len(train_data)
poison_id = random.sample(list(range(train_n)), 2)
for i in poison_id:
    print(f"Sample:{i}")
    ...
    for idx in word_to_perturb:
        ...
        for cand in candidate_ids:
            ...
            perturb_token = original_word.squeeze() #tensor size [token_num]
